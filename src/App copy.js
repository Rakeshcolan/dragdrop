...
const DnDFlow = () => {
  ...
  const onDrop = useCallback(
    (event) => {
      ...
      const parentPosition = reactFlowInstance.getNodes().find(element => element.id === parentGroupId)?.position;
      ...
      if (parentGroupId.split("_").includes("groupnode")) {
        ...
        newNode = {
          ...
          type: type,
          position,
          data: {groupId: nodeID ,nodeId:nodeID,onChangeInput: updateTextUpdaterInput,},
        };
        } else {
          const position = reactFlowInstance.project({
            x: event.clientX - reactFlowBounds.left,
